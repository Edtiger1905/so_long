...
/*   Updated: 2026/02/09 14:23:40 by epandele         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	init_game_window(t_map *map, t_game *game)
{
	...
	init_game(game, map);
	map->game = game;
	map->mlx_win = mlx_new_window(map->mlx, game->win_width, game->win_height,
			"so_long");
	if (!map->mlx_win)
	{
		exit_game(map, map->mlx);
...
int	main(int ac, char **av)
{
	...
	if (ac != 2)
		return (
			ft_printf("This program needs exactly 2 arguments to work.\n"), 0);
	init_struct(&map);
	...
	init_hooks(&map);
	ft_printf("Game started! Collect %d ichigo's hollow ", map.coin);
	ft_printf("masks to finish the game\n");
	mlx_loop(map.mlx);
	return (0);
}
